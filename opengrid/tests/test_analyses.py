...
class AnalysisTest(unittest.TestCase):
    ...
    def test_share_of_standby_1(self):
        df = pd.DataFrame(data={'conso': np.ones(48)},
                          index=pd.date_range(start=pd.Timestamp('20180304'), periods=48, freq='h'))
    ...
    def test_share_of_standby_2(self):
        df = pd.DataFrame(data={'conso': np.ones(48)}, index=pd.date_range(
            start=pd.Timestamp('20180304'), periods=48, freq='h'))
        df.iloc[0, 0] = 0
        share_of_standby = og.analysis.share_of_standby(df, resolution='24h')
